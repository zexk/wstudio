...
test "audio editor take commands target selected region" {
    var app = try arrangementApp();
    defer app.deinit();
    app.session.project.tempo_bpm = @as(f64, @floatFromInt(app.session.project.sample_rate)) * 60.0;
    app.session.engine.transport.tempo_bpm = app.session.project.tempo_bpm;
    const first = try app.session.project.addAudioSource("first", app.session.project.sample_rate, 1, &.{ 1, 1, 1, 1 });
    const second = try app.session.project.addAudioSource("second", app.session.project.sample_rate, 1, &.{ 2, 2, 2, 2 });
    try app.session.arrangement.lane(0).?.place(app.allocator, ws.Clip.initAudio(64, 64, .{
        .source_id = first,
        .source_start_frame = 0,
        .source_length_frames = 4,
        .alternate_takes = .{ .{ .source_id = second, .source_start_frame = 0, .source_length_frames = 4, .length_ticks = 64 }, null, null, null, null, null, null },
    }));
    app.view = .audio_editor;
    app.audio_track = 0;
    app.audio_clip = 0;
    ...
    commands.run(&app, "take next");
    try std.testing.expectEqual(second, app.session.arrangement.lane(0).?.clips.items[0].content.audio.source_id);
    commands.run(&app, "comp 1 1 3");
    const audio = app.session.arrangement.lane(0).?.clips.items[0].content.audio;
    try std.testing.expectEqualSlices(f32, &.{ 2, 1, 1, 2 }, app.session.project.audioSource(audio.source_id).?.samples);
}
...
test "session swap resets view, editor targets, and undo history" {
    ...
    app.drum_stamp = true;
    app.arr_stamp = true;
    app.piano_grab_delta = .{ .dstep = 1, .dpitch = 2, .moved = true };
    app.piano_mouse_draw = true;
    app.piano_erase_active = true;
    app.piano_clone_source = .{ .pitch = 60, .step = 4 };
    app.piano_mouse_grab_offset = 3;
    app.drum_paint_state = true;
    app.slicer_paint_state = false;
    app.arr_drag_bar = 2;
    app.arr_drag_resize = true;
    app.arr_drag_clone = true;
    app.arr_drag_clone_done = true;
    app.sampler_drag_marker = .start;
    app.sampler_drag_window = true;
    app.sampler_drag_norm = 0.5;
    app.automation_mouse_edit = true;
    app.automation_mouse_erase = true;
    app.fx_drag_slot = 1;
    app.piano_visual_anchor = 1;
    app.drum_visual_anchor = 2;
    app.slicer_visual_anchor = 3;
    app.arr_visual_anchor = 4;
    app.automation_visual_anchor = 5;
    app.piano_visual_pitch_anchor = 60;
    app.drum_visual_pad_anchor = 1;
    app.slicer_visual_slice_anchor = 2;
    app.arr_visual_lane_anchor = 3;
    app.piano_visual_edit = true;
    app.drum_visual_edit = true;
    app.slicer_visual_edit = true;
    app.piano_op_pending = 'd';
    app.drum_op_pending = 'y';
    app.slicer_op_pending = 'd';
    app.arr_op_pending = 'y';
    ...
    try std.testing.expect(!app.drum_stamp);
    try std.testing.expect(!app.arr_stamp);
    try std.testing.expectEqual(@TypeOf(app.piano_grab_delta){}, app.piano_grab_delta);
    try std.testing.expect(!app.piano_mouse_draw);
    try std.testing.expect(!app.piano_erase_active);
    try std.testing.expect(app.piano_clone_source == null);
    try std.testing.expectEqual(@as(u16, 0), app.piano_mouse_grab_offset);
    try std.testing.expect(app.drum_paint_state == null);
    try std.testing.expect(app.slicer_paint_state == null);
    try std.testing.expect(app.arr_drag_bar == null);
    try std.testing.expect(!app.arr_drag_resize);
    try std.testing.expect(!app.arr_drag_clone);
    try std.testing.expect(!app.arr_drag_clone_done);
    try std.testing.expect(app.sampler_drag_marker == null);
    try std.testing.expect(!app.sampler_drag_window);
    try std.testing.expectEqual(@as(f32, 0), app.sampler_drag_norm);
    try std.testing.expect(!app.automation_mouse_edit);
    try std.testing.expect(!app.automation_mouse_erase);
    try std.testing.expect(app.fx_drag_slot == null);
    try std.testing.expect(app.piano_visual_anchor == null);
    try std.testing.expect(app.drum_visual_anchor == null);
    try std.testing.expect(app.slicer_visual_anchor == null);
    try std.testing.expect(app.arr_visual_anchor == null);
    try std.testing.expect(app.automation_visual_anchor == null);
    try std.testing.expect(app.piano_visual_pitch_anchor == null);
    try std.testing.expect(app.drum_visual_pad_anchor == null);
    try std.testing.expect(app.slicer_visual_slice_anchor == null);
    try std.testing.expect(app.arr_visual_lane_anchor == null);
    try std.testing.expect(!app.piano_visual_edit);
    try std.testing.expect(!app.drum_visual_edit);
    try std.testing.expect(!app.slicer_visual_edit);
    try std.testing.expect(app.piano_op_pending == null);
    try std.testing.expect(app.drum_op_pending == null);
    try std.testing.expect(app.slicer_op_pending == null);
    try std.testing.expect(app.arr_op_pending == null);
    ...
    try std.testing.expectEqual(@as(u32, 0), app.modal.count);
    try std.testing.expect(app.modal.pending == null);
...
test "Ctrl-P and Ctrl-N cycle an active Tab completion" {
    ...
    try std.testing.expectEqualStrings("bpm 120", app.modal.cmd_buf[0..app.modal.cmd_len]);
}
...
test "random key sequences never panic in any view" {
    ...
    const specials = [_]modal_mod.Key{
        ...
        .end,      .ctrl_w,     .ctrl_a,        .ctrl_b,       .ctrl_e,       .ctrl_u,     .ctrl_k,
        .ctrl_p,   .ctrl_n,     .ctrl_y,        .history_prev, .history_next, .ctrl_r,
    };
