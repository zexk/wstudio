...
/// view's status line, followed by a dimmed, space-separated list of every
/// command name still matching what's been typed so far. A single match
/// already gets spelled out in full by Tab's completion, so the list only
/// kicks in at 2+ matches — otherwise it'd just echo back the buffer. Only
/// applies before the first space (arguments aren't completed or listed).
/// Capped at `max_chars` so a wide match set can't overflow the line.
