...
const min_cell_w: f32 = 18;

const CellRange = struct { first: usize, count: usize };

fn visibleCellRange(total: usize, cursor: usize, capacity: usize) CellRange {
    const count = @min(total, @max(capacity, 1));
    const first = @min(cursor / count * count, total - count);
    return .{ .first = first, .count = count };
}

test "long GUI step grids page around cursor" {
    try std.testing.expectEqualDeep(CellRange{ .first = 0, .count = 16 }, visibleCellRange(16, 15, 32));
    try std.testing.expectEqualDeep(CellRange{ .first = 32, .count = 32 }, visibleCellRange(100, 47, 32));
    try std.testing.expectEqualDeep(CellRange{ .first = 68, .count = 32 }, visibleCellRange(100, 99, 32));
}
...
pub fn draw(
    ...
    const total_cells: usize = @max(1, (storage_step_count + stride - 1) / stride);
    ...
    const absolute_cursor_cell = @min(@as(usize, cursor[1]) / stride, total_cells - 1);
    const range = visibleCellRange(total_cells, absolute_cursor_cell, @intFromFloat(@max(1, @floor(grid_w / min_cell_w))));
    const first_cell = range.first;
    const step_count = range.count;
    const first_tick = first_cell * stride;
    const last_tick = @min(storage_step_count, (first_cell + step_count) * stride);
    ...
    const cursor_step = absolute_cursor_cell - first_cell;
    ...
    for (0..step_count) |step| {
        const beat = (first_cell + step) / steps_per_beat;
    ...
    if (visual_anchor) |anchor_raw| {
        const anchor = @min(@as(usize, anchor_raw) / stride, total_cells - 1);
        const absolute_lo = @min(anchor, absolute_cursor_cell);
        const absolute_hi = @max(anchor, absolute_cursor_cell);
        const lo = @min(step_count, @max(first_cell, absolute_lo) - first_cell);
        const hi = @min(first_cell + step_count, absolute_hi + 1) -| first_cell;
        const x1 = grid_x + @as(f32, @floatFromInt(lo)) * cell_w;
        const x2 = grid_x + @as(f32, @floatFromInt(hi)) * cell_w;
    ...
    for (0..step_count + 1) |step| {
        const x = grid_x + @as(f32, @floatFromInt(step)) * cell_w;
        const absolute_step = first_cell + step;
        const on_beat = absolute_step % steps_per_beat == 0;
        const position_units = absolute_step * meter_denominator;
    ...
    if (play_step) |step| {
        const play_cell = step / stride;
        if (play_cell >= first_cell and play_cell < first_cell + step_count) {
            const x = grid_x + @as(f32, @floatFromInt(play_cell - first_cell)) * cell_w;
    ...
    for (row_start..row_end, 0..) |row, display_row| {
        for (first_tick..last_tick) |step| {
            ...
            const x = grid_x + @as(f32, @floatFromInt(step - first_tick)) * tick_w;
            const y = grid_y + @as(f32, @floatFromInt(display_row)) * row_h;
    ...
    for (row_start..row_end, 0..) |row, display_row| {
        const len = if (kind == .drum)
            instrument.padSteps(@intCast(row), @intCast(storage_step_count)) / stride
        else
            instrument.sliceSteps(@intCast(row), @intCast(storage_step_count)) / stride;
        if (len >= first_cell + step_count) continue;
        const x = grid_x + @as(f32, @floatFromInt(len -| first_cell)) * cell_w;
        const y = grid_y + @as(f32, @floatFromInt(display_row)) * row_h;
        draw_list.addRectFilled(.{
            .pmin = .{ x, y },
    ...
    if (hovered and mouse[1] >= grid_y and row_count > 0) {
        ...
        if (mouse[0] < grid_x) {
            ...
            const display_step = @min(step_count - 1, @as(usize, @intFromFloat((mouse[0] - grid_x) / cell_w)));
            const step = first_cell + display_step;
            const x = grid_x + @as(f32, @floatFromInt(display_step)) * cell_w;
            const y = grid_y + @as(f32, @floatFromInt(display_row)) * row_h;
            ...
            const fine_step: u16 = @intCast(@min(
                ...
                first_tick + @as(usize, @intFromFloat((mouse[0] - grid_x) / (cell_w / @as(f32, @floatFromInt(stride))))),
            ));
