...
fn drawTargetStrip(app: anytype, clip: *ws.Clip) void {
    ...
    if (zgui.beginChild("automation-targets", .{ .w = 0, .h = 52, .window_flags = .{ .no_scrollbar = true } })) {
...
pub fn drawParamPicker(app: anytype) void {
    ...
    picker.header(theme.focus, icons.param ++ "  AUTOMATE PARAM", app.core.pickerTargetName(), "", match_count, filter);
