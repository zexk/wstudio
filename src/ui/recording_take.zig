...
/// Incremental float WAV. Header lengths update after every append, so
...
pub const RecordingTake = struct {
    ...
    pub fn start(io: std.Io, nonce: u64, sample_rate: u32, channels: u16) !RecordingTake {
        ...
        wavHeader(&header, sample_rate, channels, 0);
    ...
    pub fn appendAt(self: *RecordingTake, start_frame: u64, samples: []const f32, sample_rate: u32) !void {
        if (start_frame > self.frames) {
            const silence: [512]f32 = @splat(0.0);
            var missing = start_frame - self.frames;
            while (missing > 0) {
                const count: usize = @intCast(@min(missing, silence.len / self.channels));
                try self.file.writeStreamingAll(self.io, std.mem.sliceAsBytes(silence[0 .. count * self.channels]));
                self.frames += count;
                missing -= count;
            }
        }
        ...
        var header: [44]u8 = undefined;
        wavHeader(&header, sample_rate, self.channels, self.frames);
...
fn wavHeader(out: *[44]u8, sample_rate: u32, channels: u16, frames: usize) void {
    const data_len: u32 = @intCast(@min(frames *| channels *| @sizeOf(f32), std.math.maxInt(u32) - 36));
    @memcpy(out[0..4], "RIFF");
    ...
    std.mem.writeInt(u32, out[28..32], sample_rate * @sizeOf(f32) * channels, .little);
    std.mem.writeInt(u16, out[32..34], @sizeOf(f32) * channels, .little);
...
test "partial recording is always a readable WAV" {
    var header: [44]u8 = undefined;
    wavHeader(&header, 48_000, 2, 3);
    try std.testing.expectEqual(@as(u32, 60), std.mem.readInt(u32, header[4..8], .little));
    try std.testing.expectEqual(@as(u32, 24), std.mem.readInt(u32, header[40..44], .little));
}
