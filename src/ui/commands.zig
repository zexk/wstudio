...
pub const cmds: []const cmd_mod.Def = &.{
    ...
    .{ .name = "track-instrument", .desc = "[<n>] <audio|synth|sampler|drum|slicer|soundfont|acoustic>  change track n's instrument, keeping its notes where the old and new kinds are compatible (no n: cursor track)", .run = wrap(commands_tracks.cmdTrackInstrument) },
    ...
    .{ .name = "freeze",       .desc = "render MIDI track through its FX and unload its devices", .run = wrap(commands_mixer.cmdFreeze), .scope = .{ .drum = true, .sampler = true, .synth = true, .slicer = true, .soundfont = true, .acoustic = true } },
    .{ .name = "unfreeze",     .desc = "restore devices and MIDI state on a frozen track", .run = wrap(commands_mixer.cmdUnfreeze) },
    .{ .name = "flatten",      .desc = "render MIDI track through its FX and replace it with audio", .run = wrap(commands_mixer.cmdFlatten) },
