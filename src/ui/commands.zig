...
pub const cmds: []const cmd_mod.Def = &.{
    ...
    .{ .name = "flatten",      .desc = "render MIDI track through its FX and replace it with audio", .run = wrap(commands_mixer.cmdFlatten), .scope = .{ .drum = true, .sampler = true, .synth = true, .slicer = true, .soundfont = true, .acoustic = true } },
