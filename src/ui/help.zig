...
pub fn buildHelp(t: *HelpText, cmds: []const cmd_mod.Def, keymaps: []const config_mod.Keymap) void {
    ...
    t.key("(in :) tab/S-tab", "cycle command or argument; ^Y accepts");
    t.key("(in :/) ←/→",  "move cursor; home/end or ^A/^B/^E jumps to start/start/end (^E restores completion stem)");
    ...
    t.key("(edit) [] <> ;' fF r i", "resize, field nudge/cycle, micro-nudge, reverse, invert");
