...
pub fn buildHelp(t: *HelpText, cmds: []const cmd_mod.Def, keymaps: []const config_mod.Keymap) void {
    ...
    t.key("B",            "detect the loaded clip's BPM and stretch it to the project tempo");
    ...
    t.key(":bpm-sync",    "[clip-bpm]  stretch the clip to the project tempo (detects the clip's own BPM if omitted)");
    t.key("B",            "detect the loaded clip's BPM and stretch it to the project tempo (sampler/slice only)");
