...
`persist.zig`'s `file_version` (currently 71) is the only format version
...
Version 71 adds standalone Crossover FX. Version 70 adds Utility FX loudness
compensation. Version 69 adds Utility FX
noise generation. Version 68 adds sample-accurate
delay to Utility FX. Version 67 gives each
synth envelope stage its own curve.
...
Version 62 adds the EQ's `analog` flag, which designs its peaking bands by
matched Z-transform instead of the RBJ cookbook.
