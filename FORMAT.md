...
| v36 | The factory drum kits regrouped their 16 pads by soundtype (kick, kick-2, snare, snare-2, hihat, hat-2, open, crash, clap, rim, stick, tom-1, tom-2, perc-hi, perc-lo, signature). Kit audio is regenerated by name on load, never stored, so a v35-or-older file would otherwise find a different drum under each step: those files get the kit rebuilt in the old pad order instead (`drum_kit.legacyLayout`). |
